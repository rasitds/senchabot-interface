...
export class PrintCommand implements ICommand {
    ...
    private readonly InvalidParameterMessage = "Invalid parameter. Valid parameters are: /, _";
    ...
    public execute(parameters: string) {
        ...
        if (booleanType)
            this.setResponseState(message, undefined);
        else
            this.setResponseState("....", message);
    }

    setResponseState(message?: string, error?: string) {
        this.responseContext.setResponseState({ lineText: message, outputText: [ error ] });
    }
